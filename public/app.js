...
const STORAGE_KEY = 'peppol-quest-world-hub-v1';

const zoneBlueprints = {
  'peppol-101': {
    eyebrow: 'Foundation Zone',
    mapHeading: 'Peppol 101 Meadow',
    hubTip: 'A strong BA translates business pain into a process story before they jump into technical detail.',
    wordOfDay: {
      term: 'Peppol',
    ...
    skillsAwarded: ['Value framing', 'Issue triage'],
    terrain: [
  ...
  'four-corner-city': {
    ...
    skillsAwarded: ['Process mapping', 'Ownership analysis'],
    terrain: [
    ...
    steps: [
      ...
      'Return to Archivist Ember and explain why the model matters to a BA diagnosing incidents.'
    ],
    shardMeta: {
      ...
      rowan: { title: 'Ownership clue', summary: 'Once the sender hands off to its AP, ownership of that handoff sits with Corner 2.' },
...
const glossaryTerms = [
  {
    ...
    plain: 'A shared network and rule set for exchanging business documents electronically.',
    technical: 'A four-corner interoperability framework governed through OpenPeppol specifications and local implementations.',
    ba: 'Think of it as the agreed operating model and standards layer that lets trading partners exchange structured documents reliably.'
  },
  {
    term: 'eInvoicing',
    category: 'Core concept',
    plain: 'Sending invoice data directly between systems instead of retyping or emailing files.',
    technical: 'Structured electronic invoice exchange between systems using agreed schemas, identifiers, and transport rules.',
    ba: 'It matters because it changes process design, controls, timing, exception handling, and the data model.'
  },
  {
    term: 'PINT A-NZ',
    category: 'Regional',
    plain: 'A Peppol invoice format tailored for Australia and New Zealand needs.',
    technical: 'The Peppol International invoice specification with Australia-New Zealand localisation rules and expectations.',
    ba: 'This is where local tax, policy, and implementation realities show up in scope and acceptance criteria.'
  },
  {
    term: 'Access Point',
    category: 'Network role',
    plain: 'A provider that sends and receives Peppol messages on behalf of a business.',
    technical: 'A Peppol network endpoint responsible for secure message delivery between participants.',
    ba: 'When incidents happen, the AP is often a key dependency and can change who owns the next troubleshooting step.'
  },
  {
    term: 'SMP',
    category: 'Discovery',
    plain: 'A place where systems can check what documents a participant can receive.',
    technical: 'Service Metadata Publisher: stores participant capabilities, document identifiers, and supported processes.',
    ba: 'Useful when you need to prove whether a receiver was configured to accept the document type in scope.'
  },
  {
    term: 'SML',
    category: 'Discovery',
    plain: 'A lookup layer that helps locate where a participant’s metadata lives.',
    technical: 'Service Metadata Locator: discovery infrastructure used to find the relevant SMP for a participant.',
    ba: 'When discovery fails, you may need to ask whether the participant can be located before debating payload quality.'
  },
  {
    term: 'CustomizationID',
    category: 'Document structure',
    plain: 'A label that tells systems which invoice ruleset the document follows.',
    technical: 'A document identifier used to declare the customization or business rules applied to the payload.',
    ba: 'This often appears in mapping and test conversations because the same business story may require different rulesets.'
  },
  {
    term: 'ProfileID',
    category: 'Document structure',
    plain: 'A label showing which business process the document belongs to.',
    technical: 'A business process identifier paired with document rules to signal expected interaction behaviour.',
    ba: 'Helps connect a payload to the process scenario your project is actually trying to support.'
  },
  {
    term: 'Validation',
    category: 'Quality',
    plain: 'Checking whether a document follows the required rules.',
    technical: 'Schema, code list, and business rule evaluation against the payload content.',
    ba: 'Validation failures are where requirements, mapping, and test design all collide.'
  },
  {
    term: 'Four-corner model',
    category: 'Architecture',
    plain: 'Supplier system and buyer system on the outside, access points in the middle.',
    technical: 'Corner 1 and Corner 4 are participant business systems; Corner 2 and Corner 3 are access points.',
    ba: 'This model helps you decide ownership, dependencies, escalation paths, and who should answer the next question.'
  }
];
...
const skillTracks = [
  {
    ...
    nodes: [
      { id: 'value-framing', label: 'Value framing', description: 'Explain why eInvoicing matters in business terms.', unlockedBy: ['peppol-101'] },
      { id: 'current-future', label: 'Current vs future state', description: 'Describe how work changes from PDFs to structured exchange.', unlockedBy: ['peppol-101'] },
      { id: 'stakeholder-map', label: 'Stakeholder mapping', description: 'Identify who owns which step of the operating model.', unlockedBy: ['peppol-101', 'four-corner-city'] }
    ]
  },
  {
    title: 'Technical BA Skills',
    nodes: [
      { id: 'issue-triage', label: 'Issue triage', description: 'Separate discovery, process, and validation problems.', unlockedBy: ['peppol-101'] },
      { id: 'process-map', label: 'Process mapping', description: 'Trace a message across corners and handoff points.', unlockedBy: ['four-corner-city'] },
      { id: 'ownership-analysis', label: 'Ownership analysis', description: 'Decide which corner should act next when delivery fails.', unlockedBy: ['four-corner-city'] }
    ]
  },
  {
    title: 'A-NZ Specialist Path',
    nodes: [
      { id: 'regional-scope', label: 'Regional scoping', description: 'Anticipate what A-NZ localisation changes in requirements.', unlockedBy: ['anz-lab'] },
      { id: 'gst-thinking', label: 'GST-aware design', description: 'Understand where local tax context affects invoice design.', unlockedBy: ['anz-lab'] }
    ]
  },
  {
    title: 'Consulting & Strategy',
    nodes: [
      { id: 'exec-summary', label: 'Executive summary', description: 'Turn technical lessons into clear decision-ready language.', unlockedBy: ['peppol-101'] },
      { id: 'roadmap-thinking', label: 'Roadmap thinking', description: 'Sequence learning zones into a sensible adoption journey.', unlockedBy: ['peppol-101', 'four-corner-city'] }
    ]
  }
];
...
const els = {
  ...
  shardsCard: document.querySelector('#shardsCard'),
...
function zoneProgressPercent(zoneState) {
  ...
  const ratios = [8, 30, 58, 82];
  return ratios[zoneState.questStep] || 0;
}
...
function renderHubCards() {
  els.zoneGrid.innerHTML = state.modules
    .map((module) => {
...
function renderProfileSummary() {
  const completed = completedPlayableCount();
  const totalPlayable = Object.keys(zoneBlueprints).length;
  const rank = currentRank();
  const shards = totalShards();

  els.navRank.textContent = rank;
  els.navShards.textContent = String(shards);
  els.rankLabel.textContent = rank;
  els.zoneCompletionLabel.textContent = `${completed} / ${totalPlayable}`;
...
function collectShard(zoneKey, shardKey, lesson) {
  ...
  if (!zoneState.shards.includes(shardKey)) {
    zoneState.shards.push(shardKey);
  }
...
function handlePeppol101(actorKey) {
  ...
  if (actorKey === 'mayor') {
    ...
    setDialogue(
      'Mayor Myrtle: A supplier says, “I sent the invoice.” The buyer says, “I never received it.” In Peppol, which corners represent the two business systems?',
      [
        {
          label: 'Corner 1 and Corner 4',
          onSelect: () => {
            collectShard('peppol-101', 'mayor', blueprint.shardMeta.mayor.summary);
            setDialogue(
              'Correct. The businesses live at Corner 1 and Corner 4. Corners 2 and 3 are the access points that carry the message between them. Now go see the Access Point Ranger.',
              [],
              actor
            );
            renderZone();
        ...
        {
          label: 'Corner 2 and Corner 3',
          onSelect: () => setDialogue('Not quite. Corners 2 and 3 are the access points, not the business systems.', [], actor)
        }
      ],
      actor
    );
  ...
  if (actorKey === 'ap') {
    ...
    setDialogue(
      'Access Point Ranger: The participant could not be discovered. What kind of issue is that most likely?',
      [
        {
          label: 'Routing or discovery issue',
          onSelect: () => {
            collectShard('peppol-101', 'ap', blueprint.shardMeta.ap.summary);
            if (zoneState.questStep === 1) zoneState.questStep = 2;
            setDialogue(
              'Exactly. If the participant cannot be discovered, a BA should check identifiers, discovery metadata, or receiver capability setup before blaming the invoice content.',
              [],
              actor
            );
            renderZone();
        ...
        {
          label: 'Printer jam in accounts payable',
          onSelect: () => setDialogue('Nope. That would be a local paper-process problem, not a discovery problem in the network.', [], actor)
        }
      ],
      actor
    );
  ...
  if (actorKey === 'buyer') {
    ...
    setDialogue(
      'Buyer Clerk Nia: A PDF arrived by email, but no structured data entered the finance system. What should a BA say?',
      [
        {
          label: 'That is not Peppol eInvoicing yet',
          onSelect: () => {
            collectShard('peppol-101', 'buyer', blueprint.shardMeta.buyer.summary);
            if (zoneState.questStep === 2) zoneState.questStep = 3;
            setDialogue(
              'Right. A PDF may still be a manual document workflow. Peppol eInvoicing means structured software-to-software data exchange, not just “an invoice that arrived electronically.”',
              [],
              actor
            );
            renderZone();
        ...
        {
          label: 'That counts as full automation',
          onSelect: () => setDialogue('Not yet. It is still a document handoff rather than structured exchange into the system.', [], actor)
        }
      ],
      actor
    );
  ...
  if (actorKey === 'validator') {
    ...
    setDialogue(
      'Validator Vale: The participant is found, but the invoice fails because required values do not match the rules. What category is the problem?',
      [
        {
          label: 'Validation or business-rule issue',
          onSelect: () => {
            collectShard('peppol-101', 'validator', blueprint.shardMeta.validator.summary);
            zoneState.completed = true;
            zoneState.questStep = 4;
            updateCompletion('peppol-101');
            showCompletion('peppol-101');
            renderZone();
          }
        },
        {
          label: 'Wrong office chair',
          onSelect: () => setDialogue('Funny, but this one is a payload validation issue.', [], actor)
        }
      ],
      actor
    );
...
function handleFourCornerCity(actorKey) {
  ...
  if (actorKey === 'sender') {
    if (zoneState.questStep === 0) zoneState.questStep = 1;
    setDialogue(
      'Sender Sage: In the four-corner model, where does the supplier business system live?',
      [
        {
          label: 'Corner 1',
          onSelect: () => {
            collectShard('four-corner-city', 'sender', blueprint.shardMeta.sender.summary);
            setDialogue(
              'Correct. The supplier system is Corner 1, handing the message to its access point in Corner 2.',
              [],
              actor
            );
            renderZone();
        ...
        {
          label: 'Corner 3',
          onSelect: () => setDialogue('Not quite. Corner 3 is the receiver-side access point, not the supplier system.', [], actor)
        }
      ],
      actor
    );
  ...
  if (actorKey === 'ada') {
    ...
    setDialogue(
      'Access Point Ada: Which path shows the normal journey of a Peppol document?',
      [
        {
          label: 'Corner 1 → Corner 2 → Corner 3 → Corner 4',
          onSelect: () => {
            collectShard('four-corner-city', 'ada', blueprint.shardMeta.ada.summary);
            if (zoneState.questStep === 1) zoneState.questStep = 2;
            setDialogue(
              'Exactly. Supplier business system to supplier access point, then to buyer access point, then into the buyer business system.',
              [],
              actor
            );
            renderZone();
        ...
        {
          label: 'Corner 1 → Corner 4 → Corner 2 → Corner 3',
          onSelect: () => setDialogue('No. The business systems do not jump over the access points in the middle.', [], actor)
        }
      ],
      actor
    );
  ...
  if (actorKey === 'rowan') {
    ...
    setDialogue(
      'Receiver Rowan: The sender system already handed the message to its access point, but the next handoff fails. Which corner owns that handoff?',
      [
        {
          label: 'Corner 2',
          onSelect: () => {
            collectShard('four-corner-city', 'rowan', blueprint.shardMeta.rowan.summary);
            if (zoneState.questStep === 2) zoneState.questStep = 3;
            setDialogue(
              'Yes. Once the supplier system has handed off, the next operational question sits with the sender-side access point in Corner 2.',
              [],
              actor
            );
            renderZone();
        ...
        {
          label: 'Corner 1',
          onSelect: () => setDialogue('Not the best answer. Corner 1 already completed its handoff in this scenario.', [], actor)
        }
      ],
      actor
    );
  ...
  if (actorKey === 'archivist') {
    ...
    setDialogue(
      'Archivist Ember: Why does the four-corner model matter to a BA during incident diagnosis?',
      [
        {
          label: 'It shows where ownership changes and who should answer the next question',
          onSelect: () => {
            collectShard('four-corner-city', 'archivist', blueprint.shardMeta.archivist.summary);
            zoneState.completed = true;
            zoneState.questStep = 4;
            updateCompletion('four-corner-city');
            showCompletion('four-corner-city');
            renderZone();
          }
        },
        {
          label: 'It is mostly decorative architecture jargon',
          onSelect: () => setDialogue('No. It is useful precisely because it clarifies responsibility and escalation paths.', [], actor)
        }
      ],
      actor
    );
...
function interactWithActor(actorKey) {
  const zoneKey = state.player.currentZone;
  if (zoneKey === 'peppol-101') {
    handlePeppol101(actorKey);
    return;
  }
  if (zoneKey === 'four-corner-city') {
    handleFourCornerCity(actorKey);
  }
}

function nearbyActor(zoneState, blueprint) {
  return Object.entries(blueprint.actors).find(([, actor]) => {
    return Math.abs(actor.x - zoneState.player.x) + Math.abs(actor.y - zoneState.player.y) <= 1;
  });
}

function tileMarker(zoneState, blueprint, x, y) {
  if (zoneState.player.x === x && zoneState.player.y === y) {
    return '<div class="marker player">Y</div>';
  }

  for (const actor of Object.values(blueprint.actors)) {
    if (actor.x === x && actor.y === y) {
      return `<div class="marker npc">${actor.icon}</div>`;
    }
  }

  if (x === 6 && y === 7 && zoneState.completed) {
    return '<div class="marker goal">✓</div>';
  }
...
function renderZoneGrid() {
  ...
  els.gameGrid.innerHTML = blueprint.terrain
    .map((row, y) =>
      row
        .map((type, x) => {
          return `
...
function renderQuestLog() {
  ...
  els.currentQuestText.textContent = zoneState.completed
    ? 'Zone complete — replay any conversation or return to the map.'
    : blueprint.steps[Math.min(zoneState.questStep, blueprint.steps.length - 1)];
  ...
  els.questSteps.innerHTML = blueprint.steps
    .map((step, index) => {
  ...
  if (!zoneState.shards.length) {
    ...
    els.shardList.innerHTML = zoneState.shards
      .map((shardKey) => {
        const shard = blueprint.shardMeta[shardKey];
        return `
          <article class="shard-pill">
            <strong>${shard.title}</strong>
            <span>${shard.summary}</span>
          </article>
        `;
      })
      .join('');
...
function renderGlossary() {
  ...
  const filtered = glossaryTerms.filter((entry) => {
    const haystack = `${entry.term} ${entry.category} ${entry.plain} ${entry.technical} ${entry.ba}`.toLowerCase();
...
function renderSkillTree() {
  ...
  els.skillTree.innerHTML = skillTracks
    .map((track) => {
      ...
      return `
        <section class="skill-track shell-card">
          <h3>${track.title}</h3>
          <div class="skill-nodes">${nodes}</div>
        </section>
      `;
    })
    .join('');
...
function openZone(zoneKey) {
  ...
  if (zoneState.completed) {
    setDialogue('You have already cleared this zone. Replay the conversations, or return to the world map and choose another area.', [], null);
  } else if (zoneState.questStep === 0 && !zoneState.shards.length) {
    setDialogue('Walk to the first guide and press E to begin the zone quest.', [], null);
  } else {
    setDialogue(
      `Welcome back to ${moduleForZone(zoneKey)?.title || blueprint.mapHeading}. Continue from your current quest step.`,
      [],
      null
    );
  }
...
function resetZone(zoneKey = state.player.currentZone) {
  ...
  renderHub();
  renderSkillTree();
  setDialogue('Zone reset. Walk to the first guide and press E to begin again.', [], null);
  saveLocal();
}
...
function interact() {
  ...
  if (!found) {
    setDialogue('There is nobody close enough to talk to. Move next to a guide and press E.', [], null);
    return;
  }

  const [actorKey, actor] = found;
  zoneState.lastNpc = actorKey;
  interactWithActor(actorKey);
}
...
function buildSavePayload() {
  return {
    ...
    progress: state.modules
      .filter((module) => zoneBlueprints[module.slug])
      .map((module) => {
...
async function saveProfile() {
  try {
    ...
    renderProfileSummary();
    renderQuestLog();
    setDialogue('Progress saved. Your zone progress is now synced to the profile endpoint.', [], null);
  } catch {
    setDialogue('Could not save to the API, but your progress is still saved locally in this browser.', [], null);
  }
}

function attachEvents() {
  els.navLogo.addEventListener('click', () => {
    showScreen('hub');
    renderHub();
  });
  ...
  els.glossarySearch.addEventListener('input', (event) => {
