...
function progressPercent() {
  ...
  return [10, 35, 65, 85][game.questStep] || 0;
}
...
function interact() {
  ...
  if (key === 'mayor') {
    ...
    setDialogue(
      ...
      [
        { label: 'Corner 1 and Corner 4', onSelect: () => { completeShard('mayor', 'The supplier and buyer business systems are Corner 1 and Corner 4.'); setDialogue('Correct. The businesses live at Corner 1 and Corner 4. The access points are Corners 2 and 3. Now go see the Access Point Ranger.'); render(); } },
        { label: 'Corner 2 and Corner 3', onSelect: () => setDialogue('Not quite. Corners 2 and 3 are the access points, not the businesses.') }
      ]
    );
    return;
  }
  ...
  if (key === 'ap') {
    if (game.questStep < 1) {
      setDialogue('Access Point Ranger: Start with Mayor Myrtle first.');
      return;
    }
    setDialogue(
      'Access Point Ranger: The participant could not be discovered. What kind of issue is that most likely?',
      [
        { label: 'Routing / discovery issue', onSelect: () => { completeShard('ap', 'Discovery failures usually point to routing or participant lookup issues.'); if (game.questStep === 1) game.questStep = 2; setDialogue('Exactly. If the participant cannot be discovered, the BA should inspect identifiers, SMP/SML metadata, or receiver capability setup. Now visit Buyer Clerk Nia.'); render(); } },
        { label: 'Printer jam in accounts payable', onSelect: () => setDialogue('Funny, but no. This is a network/discovery issue, not a physical invoice handling problem.') }
      ]
    );
    return;
  }
  ...
  if (key === 'buyer') {
    if (game.questStep < 2) {
      setDialogue('Buyer Clerk Nia: You still need the routing clue from the Access Point Ranger.');
      return;
    }
    setDialogue(
      'Buyer Clerk Nia: A PDF arrived by email, but no structured data entered the finance system. What should a BA say?',
      [
        { label: 'That is not Peppol eInvoicing yet', onSelect: () => { completeShard('buyer', 'A PDF by email is not the same as structured Peppol eInvoicing.'); if (game.questStep === 2) game.questStep = 3; setDialogue('Right. A PDF can still be manual. Peppol eInvoicing is structured software-to-software data exchange. Now return to Validator Vale for the final diagnosis.'); render(); } },
        { label: 'That counts as full automation', onSelect: () => setDialogue('Nope. Receiving a PDF is still a document workflow, not structured exchange.') }
      ]
    );
    return;
  }
  ...
  if (key === 'validator') {
    if (game.questStep < 3) {
      setDialogue('Validator Vale: Gather the earlier clues before trying the final diagnosis.');
      return;
    }
    setDialogue(
      ...
      [
        ...
        { label: 'Wrong office chair', onSelect: () => setDialogue('Afraid not. This is a payload validation issue, not office furniture.') }
      ]
    );
  }
}
...
async function saveProfile() {
  ...
  try {
    ...
    setDialogue('Progress saved. Your first playable quest is now stored.');
  } catch {
    setDialogue('Could not save to the API, but the quest still works locally in your browser.');
  }
}

function resetGame() {
  game = initialGame();
  setDialogue('Quest reset. Walk over to Mayor Myrtle and press E to begin again.');
  render();
}
...
loadProfile().finally(() => {
  render();
  setDialogue('Welcome to Peppol 101 Meadow. Start by walking to Mayor Myrtle on the stone path and press E.');
});
